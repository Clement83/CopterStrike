...
void fnctn_initPlayer() {
  ...
  player.x_world = HELIPORT_XWORLD - (SCREEN_HALF_WIDTH) + 4;
  player.y_world = HELIPORT_YWORLD - (SCREEN_HALF_HEIGHT) - 4;
...
void fnctn_moveunit(byte i) {
  ...
  int dist = (abs(PORTEAVION_XWORLD - mobilUnit_hostile[i].x_world) + abs(PORTEAVION_YWORLD - mobilUnit_hostile[i].y_world));
  if (mobilUnit_hostile[i].life > 0 && dist<100 && dist>30 ) {
    check02 = 8;

    if (mobilUnit_hostile[i].x_world + (mobilUnit_hostile[i].width / 2) > PORTEAVION_XWORLD + 10) {
    ...
    if (mobilUnit_hostile[i].y_world + (mobilUnit_hostile[i].height / 2) < PORTEAVION_YWORLD - 8) {
...
void fnctn_initEnnemyFire() {
  ...
  for (i = 0; i < nbBuilding_Hostile; i++) {
    ...
    for (j = 0; j < MAXBULLET; j++) {
      if (building_hostile[i].fireTimer < 1 && abs(PORTEAVION_XWORLD - coordx) < 75 && abs(PORTEAVION_YWORLD - coordy) < 75 &&  building_hostile[i].life > 0) {
        if (bullet[j].distance == 0) {
          ...
          bullet[j].distance = 1;
          bullet[j].dir = 0;
          if (PORTEAVION_XWORLD > coordx + 5 && PORTEAVION_YWORLD > coordy + 5)                            {
            bullet[j].dir = 1;
          }
          if (PORTEAVION_XWORLD > coordx - 6 && PORTEAVION_XWORLD < coordx + 6 && PORTEAVION_YWORLD > coordy)   {
            bullet[j].dir = 2;
          }
          if (PORTEAVION_XWORLD < coordx - 5 && PORTEAVION_YWORLD > coordy + 5)                            {
            bullet[j].dir = 3;
          }
          if (PORTEAVION_XWORLD < coordx   && PORTEAVION_YWORLD > coordy - 6 && PORTEAVION_YWORLD < coordy + 6) {
            bullet[j].dir = 4;
          }
          if (PORTEAVION_XWORLD < coordx - 5 && PORTEAVION_YWORLD < coordy - 5)                            {
            bullet[j].dir = 5;
          }
          if (PORTEAVION_XWORLD > coordx - 6 && PORTEAVION_XWORLD < coordx + 6 && PORTEAVION_YWORLD < coordy - 6) {
            bullet[j].dir = 6;
          }
          if (PORTEAVION_XWORLD > coordx + 5 && PORTEAVION_YWORLD < coordy - 5)                            {
            bullet[j].dir = 7;
  ...
  for (i = 0; i < 20; i++) {
    ...
    for (j = 0; j < MAXBULLET; j++) {      
       if (mobilUnit_hostile[i].fireTimer < 1 && abs(PORTEAVION_XWORLD - coordx) < 75 && abs(PORTEAVION_YWORLD - coordy) < 75 &&  building_hostile[i].life > 0) {
        if (bullet[j].distance == 0) {      
          bullet[j].shooter = i;
          bullet[j].x_world = coordx;
          bullet[j].y_world = coordy;
...
/*
    if (Camion.animDamage == 0) {
      //bullet[j].distance = 0;
      if (gb.collidePointRect(bullet[j].x_world, bullet[j].y_world, PORTEAVION_XWORLD, PORTEAVION_YWORLD, 14, 14)) {
        if (Camion.life > 0) {
